...
abstract class GenerateScreenTask : DefaultTask() {
  ...
  fun execute() {
    ...
    // Step 1: Build dependency info list
    val dependencies = buildDependencyInfoList()

    // Step 2: Generate catalog using core builder
    val catalog = LicenseCatalogBuilder.build(dependencies)

    // Step 3: Write YAML to build directory
    ...
    // Step 4: Resolve catalog to licenses using core resolver
    val resolvedLicenses = LicenseCatalogResolver.resolve(catalog)

    // Step 5: Generate AppLicenses code
    ...
    // Step 6: Generate OpenSourceLicensesActivity code and layout XML
    ...
    // Step 7: Generate AndroidManifest.xml
